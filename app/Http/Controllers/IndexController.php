...
class IndexController extends Controller {
	...
	*/
	...
	public function index()
	{
		...
		$vatlieu = DB::table('lienket')->where('com','vatlieu')->orderBy('id','desc')->get();
		$partners = DB::table('partner')->where('status',1)->orderBy('id','desc')->get();
		$about_cus = DB::table('about')->where('com','ykien')->first();
		...
		$categories = DB::table('news_categories')->where('status',1)->where('noibat',1)->where('com','du-an')->orderBy('stt', 'asc')->take(6)->get();
		$home = DB::table('news_categories')->where('status',1)->where('home',1)->where('com','du-an')->orderBy('stt', 'asc')->take(5)->get();
		$com = 'index';
		// End cấu hình SEO
		$img_share = asset('upload/hinhanh/'.$setting->photo);

		return view('templates.index_tpl', compact('com','about','tintuc_moinhat','keyword','description','title','img_share','slider','news','partners','slogans','whys', 'vatlieu','about_cus','categories','home'));
	}
	...
	{
		$about = DB::table('about')->where('com','gioi-thieu')->first();
		$tamnhin = DB::table('about')->where('com','tamnhin')->first();
		$dinhhuong = DB::table('about')->where('com','dinhuong')->first();
		...
		return view('templates.about_tpl', compact('about','keyword','description','title','img_share','com','slogans','tamnhin','dinhhuong','feedbacks'));
	}
	...
	public function getNews()
	{
		...
		$about = DB::table('about')->where('com','tin-tuc')->first();		
		$com='tin-tuc';
		// Cấu hình SEO
		$title = "Tin tức";
		$keyword = "Tin tức";
		...
		$img_share = '';
		// End cấu hình SEO
		return view('templates.news_tpl', compact('tintuc','keyword','description','title','img_share','com','cateNews','about'));
	}
	...
	public function getNewsDetail($id)
	{
		...
		if(!empty($news_detail)){			
			$cate_pro = DB::table('product_categories')->where('status',1)->where('parent_id',0)->orderby('id','asc')->get();
			$baiviet_khac = DB::table('news')->where('status',1)->where('id','<>', $news_detail->id)->where('com','tin-tuc')->inRandomOrder()->take(2)->get();
			// dd($baiviet_khac);
			$com='tin-tuc';
			$setting = Cache::get('setting');
			// Cấu hình SEO
	...
	public function listProject($alias)
	{
		...
		return view('templates.listduan', compact('projects','title','description', 'keyword','cate', 'cateChilds'));
	}
	...
	{		
		...
		$projectOther = DB::table('news')->where('status', 1)->where('com', $project->com)->where('cate_id', $project->cate_id)->where('id','<>', $project->id)->take(4)->get();
	...
	{
		$data = DB::table('lienket')->where('com','vatlieu')->orderBy('id','desc')->get();
		$about = DB::table('about')->where('com','vatlieu')->first();
		$title = 'Vật liệu dự án';
		return view('templates.vatlieu',compact('about', 'title','data'));
	}
